...
class SlidingWindowSplitter(BaseTokenSplitter):
    ...
    def __init__(self, min_tokens: int = 64 , max_tokens: int = 512, overlap_tokens: int = 32, encoding_name: str = "cl100k_base"):
        super().__init__(encoding_name)
        self.min_tokens = min_tokens
        self.max_tokens = max_tokens
    ...
    def __wrapped__(self, txt: str) -> list[tuple[str, dict]]:
        ...
        while i < len(tokens):
            ...
            i += len(self._tokenize(chunk)) - self.overlap_tokens
            output.append((chunk, {}))
