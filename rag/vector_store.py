...
"""
        ...
        # metadata = docs._metadata.value
        # print("metadata: ",metadata)
        # @pw.udf
        # def parse_doc(data: bytes, metadata) -> list[pw.Json]:
        #     metadata = metadata.value
        #     if metadata['path'].endswith(".txt"):
        #         self.parser = _unwrap_udf(ParseUtf8())
        ...
        def parse_doc(data: bytes, metadata) -> list[pw.Json]:
            ...
            # Initialize variables for the different parsers
            utf8_parser = _unwrap_udf(ParseUtf8())
            unstructured_parser = _unwrap_udf(ParseUnstructured())
            open_parse_parser = _unwrap_udf(OpenParse(table_args={"parsing_algorithm": "pymupdf",}))

            # Determine which parser to use based on file type in metadata
            rets = []

            # Case 1: Use ParseUtf8 for .txt files
            if metadata.get('path', '').endswith(".txt"):
                print("Using ParseUtf8 parser")
                rets = utf8_parser(data)
            
            # Case 2: Use ParseUnstructured for general text files, .doc, .docx, etc.
            elif metadata.get('path', '').endswith((".doc", ".docx", ".html")):
                print("Using ParseUnstructured parser")
                rets = unstructured_parser(data)

            # Case 3: Use OpenParse for PDFs and similar document types
            elif metadata.get('path', '').endswith(".pdf"):
                print("Using OpenParse parser")
                rets = open_parse_parser(data)
            
            else:
                print("Unsupported file type. No parser applied.")
                return []  # or handle unsupported file types as needed

            # Process and return results with updated metadata
            return [dict(text=ret[0], metadata={**metadata, **ret[1]}) for ret in rets]  # type: ignore
